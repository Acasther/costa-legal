...
export const Sidebar = ((props) => {
  const {pages, languages, activeTab, lang, handleLanguage} = props;

  return (
    <div className='sidebar-container'>
      <Stack direction='column' spacing={4} className='sidebar-stack'> 
        {pages.map((page) => (
          <Link href={page.link} underline="none" className='menuItem' color='inherit' sx={{ color: page.id === activeTab ? '#6695fa' : '#FFFFFF' }} key={page.name}>
            {page.name}
          </Link>
        ))}
      </Stack>
